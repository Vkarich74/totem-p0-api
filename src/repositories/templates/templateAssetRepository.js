...
CREATE INDEX IF NOT EXISTS idx_tia_owner ON public.template_image_assets(owner_type, owner_slug);
CREATE INDEX IF NOT EXISTS idx_tia_slot ON public.template_image_assets(owner_type, owner_slug, slot_type);
CREATE INDEX IF NOT EXISTS idx_tia_slot_order ON public.template_image_assets(owner_type, owner_slug, slot_type, slot_index);
CREATE INDEX IF NOT EXISTS idx_tia_active ON public.template_image_assets(is_active);
`;
...
export async function listTemplateAssets(db, ownerType, ownerSlug, filters = {}){
  ...
  let sql = `
    SELECT *
    FROM public.template_image_assets
...
export async function createTemplateAsset(db, ownerType, ownerSlug, asset){
  ...
  const result = await db.query(`
    INSERT INTO public.template_image_assets (
...
export async function updateTemplateAsset(db, ownerType, ownerSlug, assetId, patch){
  ...
  const result = await db.query(`
    UPDATE public.template_image_assets
