...
SELECT
  ...
  COUNT(*) FILTER (WHERE provider = 'direct' AND payment_status = 'pending' AND payment_is_active = true)::int AS cash_pending_exposure_count,
  ...
  COALESCE(SUM(payment_amount) FILTER (WHERE provider = 'direct' AND payment_status = 'pending' AND payment_is_active = true), 0) AS cash_pending_exposure_amount,
...
filters,
  ...
  cash_pending_exposure_count: Number(summary.cash_pending_exposure_count || 0),
  ...
  cash_pending_exposure_amount: summary.cash_pending_exposure_amount || 0,
